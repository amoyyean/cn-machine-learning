...
# 加载彩色（通道顺序为BGR）图像
img = cv2.imread(human_files[6]) # It was human_files[3] in original version.
...
print('{:.2f}% of human faces is detected in the first 100 human files.'.format(obj_detect(face_detector, human_files_short) * 100))
print('{:.2f}% of human faces is detected in the first 100 dog files.'.format(obj_detect(face_detector, dog_files_short) * 100))
...
# 
# __回答:__
# 这样的要求在实际使用中不合理，用户的人脸图像并不一定包含清晰的面部特征。采用CNN或OpenCV能够在图像中没有清晰的面部特征的情况下也能检测到人脸。
# 
...
print('{:.2f}% of dog images is detected in the first 100 human files.'.format(obj_detect(dog_detector, human_files_short) * 100))
print('{:.2f}% of dog images is detected in the first 100 dog files.'.format(obj_detect(dog_detector, dog_files_short) * 100))
...
# 
# __回答:__ 
# 根据上图搭建，采用卷积->最大池化层->卷积->最大池化层->卷积->最大池化层->GAP层-全连接的步骤。
# 
# 卷积网络的过滤器(filter)不断增加，分别是16， 32， 64。
# 
# 卷积网络后使用最大池化层减小维度/参数。最后一个GAP层可以减少参数降低计算量。
# 
# 每个最大池化层和GAP层后采用Dropout，防止过拟合。
# 
# 除了最后1层都采用ReLu激活函数，它可以比较有效地防止梯度消失。
# 
# 最后的输出层使用了softmax函数，用于分类输出。
...
history_VGG = VGG16_model.fit(train_VGG16, train_targets, 
...
# 
# __回答:__ 
# 迁移学习的好处是可以充分利用成熟的预训练模型已经经过很多尝试和调整好的模型结构及相应的超参数。狗类数据集较小，与ImageNet相似度较高。因此只需要替换最后的池化层或连接层来针对新的数据集进行分类即可。第3步中的模型因为在结构和超参数配置方面都只是凭个人有限的经验的初步尝试，和经过专业人员大量训练得到的模型有较大差距。
# 
# 采用迁移学习，Xception根据参考论文是Inception的改进版，也是上述可选的Keras预训练网络中最新发布的，因此选择它尝试，有限的时间内我其实不清楚VGG,ResNet,Inception和Xception在使用场景上真正的差别在哪。将Xception的最后一层卷积层的输出作为输入，充分利用前面的预训练好的模型来发现图像各级特征，然后使用GAP层减少参数和防止过拟合，再用激活函数为softmax的全连接层作为输出层。
# 
# 根据第一次提交模型的训练历史的准确率和损失函数值的变化曲线，Xception迁移学习模型在只训练不超过3次就开始出现过拟合的情况。因此按照助教推荐的参考文献作了一些调整。谢谢助教分享优秀的资料。
...
Xception_model.compile(loss='categorical_crossentropy', optimizer=optimizers.SGD(lr=1e-4, momentum=0.9), metrics=['accuracy'])
...
history_Xception = Xception_model.fit(train_Xception, train_targets, 
          validation_data=(valid_Xception, valid_targets),
          epochs=40, batch_size=16, callbacks=[checkpointer], verbose=1)
...
# summarize history for accuracy
plt.plot(history_Xception.history['acc'])
plt.plot(history_Xception.history['val_acc'])
plt.title('model accuracy')
plt.ylabel('accuracy')
plt.xlabel('epoch')
plt.legend(['train', 'test'], loc='upper left')
...
def dog_breed_detector_Xception(img_path):
    ...
    if face_detector(img_path):
        # plt.title("The human face in the image looks like a %s." % Xception_predict_breed(img_path));
        print("The human face in the image looks like a %s." % Xception_predict_breed(img_path))
    else:
        if dog_detector(img_path):
            # plt.title("The dog is most likely a %s." % Xception_predict_breed(img_path));
            print("The dog is most likely a %s." % Xception_predict_breed(img_path))
        else:
            print('There is neither human face nor dog detected in the image.')
    # plt.show()
...
# __回答:__
# 
# 1. 输出结果和预想的差不多，有1张狗的图片分错了，考虑到这2个寒带地区的狗品种本来基因就有相近之处，分辨出来更难一些，对人来说也是如此。
# 2. 改进模型的方法有：
# 
# 
# * 采用不同的optimizer，比如SGD(lr=1e-4, momentum=0.9);
# * 降低学习率(减小batch size);
# * 采用图片增强来扩大数据集;
# * 采用更激进地dropout来降低过拟合;
# * 采用L1或L2正则化(权重衰减)来降低过拟合;
# * 对更底层的CNN层进行微调并防止过拟合;
# * 过拟合不严重时可以适当增加epoch(训练轮次);
# * 采用助教推荐的OpenFace等更高效的新的分类器.
...
# 还没找到一个Jupyter Notebook Cell显示多张人脸或狗图片的方法。
# 可能需要把img_path或对象赋值给数组再用循环来展示。
dog_breed_detector_Xception('human face 1.jpeg')


# In[37]:


dog_breed_detector_Xception('human face 2.jpeg')


# In[38]:


dog_breed_detector_Xception('German Shepherd Dog.jpeg')


# In[39]:


dog_breed_detector_Xception('Siberian Husky.jpeg')


# In[40]:


dog_breed_detector_Xception('Beagle.jpeg')


# In[41]:


dog_breed_detector_Xception('Labrador Retriever.jpeg')
